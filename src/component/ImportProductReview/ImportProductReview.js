...
import useImportProductReview from "../../hooks/useImportProductReview";
import FileUpload from "../../lib/FileUpload/FileUpload";

const ImportProductReview = () => {
    const { customer_sid } = useParams();

    const { importFields, postImportProductReview, reviewForTable } = useImportProductReview({ customer_sid });
            ...
            <Box my="4rem" mx="1rem">
                <Flex alignItems="center" justifyContent="center" flexDir="column" gap="2rem">
                    <Flex justifyContent="flex-end" alignItems="flex-end" px="3rem" flex="auto" w="100%">
                        <FileUpload
                            id="import__product__reviews__"
                            btnText="Upload csv"
                            onUpload={({ file }) => postImportProductReview({ file })}
                        />
                    </Flex>
                    <TableContainer>
...
export default ImportProductReview;